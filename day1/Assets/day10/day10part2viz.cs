...
public class day10part2viz : MonoBehaviour
{
    ...
    void VisualizeField()
    {
        for (int i = 0; i < field.GetLength(0); i++)
        {
            for (int j = 0; j < field.GetLength(1); j++)
            {
                ...
                Instantiate(cube, pos + Vector3.down, Quaternion.identity);
                switch (field[i,j])
                {
                    case '.':
                        break;
                    case '-':
                        Instantiate(straightPipe, pos, Quaternion.Euler(90,0,0));
                        break;
                    case '|':
                        Instantiate(straightPipe, pos, Quaternion.Euler(90,90,0));
                        break;
                    case '7':
                        Instantiate(curvedPipe, pos, Quaternion.Euler(90,90,0));
                        break;
                    case 'F':
                        Instantiate(curvedPipe, pos, Quaternion.Euler(90,180,0));
                        break;
                    case 'L':
                        Instantiate(curvedPipe, pos, Quaternion.Euler(90,270,0));
                        break;
                    case 'J':
                        Instantiate(curvedPipe, pos, Quaternion.Euler(90,0,0));
                        break;
                    default:
                        break;
                }
